...
class SimplePwmSensorTest:
    ...
    def read_ina_identity(self, device: str) -> tuple[int, int]:
        """
        Read INA229 identification registers.

        Expected INA229 values:
        - MANUFACTURER_ID at 0x3E: 0x5449
        - DEVICE_ID at 0x3F: 0x2290
        """
        manufacturer_id = self.read_reg(REG_MANUFACTURER_ID, 2, device)
        device_id = self.read_reg(REG_DEVICE_ID, 2, device)
